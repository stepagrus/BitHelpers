...
public class BigEndianConverter : IByteConverter
{
	...
	{
		CheckBoundaries(buffer, offset, sizeof(double));

		return InternalConverter.UInt64ToDouble(ToUInt64(buffer, ref offset));
	}
	...
	{
		CheckBoundaries(buffer, sizeof(double));

		return InternalConverter.UInt64ToDouble(ToUInt64(buffer));
	}
