namespace BitHelpers.Tests.BigEndianConverterTests
{
  internal class ToInt32
  {
    ...
    {
      Assert.Throws<ArgumentException>(() => BigEndianConverter.ToInt32(bytes));
    }

    private static readonly object[] read_value_args = new object[]
    {
      new object[] { new byte[] {0xff, 0xff, 0xff, 0xfe}, (int) -2 },
      new object[] { new byte[] {0x00, 0x00, 0x00, 0x01}, (int) 1 },
    };
    [TestCaseSource(nameof(read_value_args))]
    public void read_value(byte[] bytes, int expected)
    {
      var value = BigEndianConverter.ToInt32(bytes);
